...
MPacket::MPacket(mtime_t time, int16_t* p, size_t ps)
    : timestamp(time),
      payload(p),
      payload_size(ps),
...
int MPacket::remaining() { return payload_size - play_head; }
...
PaTime MPacket::get_pa_timestamp() { return pa_timestamp; }

void MPacket::set_pa_timestamp(PaTime pt) { pa_timestamp = pt; }

size_t MPacket::get_payload_size() { return payload_size; }

int16_t* MPacket::get_payload() { return payload; }

mtime_t MPacket::get_timestamp() { return timestamp; }
...
void MPacket::print() {
  std::cout << "<MPacket timestamp: " << timestamp
            << " payload size: " << payload_size << " PaTime: " << pa_timestamp
            << ">" << std::endl;
}

void MPacket::print_all() {
  std::cout << "<MPacket timestamp: " << timestamp
            << " payload size: " << payload_size << " data: ";
