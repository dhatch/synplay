...
TPacket::TPacket(TPacketType ptt, mtime_t f_sent_, mtime_t t_recvd_,
                 mtime_t t_sent_, mtime_t f_recvd_, mtime_offset_t offset_)
    : tp_type(ptt),
      from_sent(f_sent_),
      to_recvd(t_recvd_),
      to_sent(t_sent_),
...
TPacket *TPacket::unpack(const uint8_t *buf, std::size_t size) {
  ...
  switch (buf[0]) {
    ...
    case FINAL:
      ...
      break;
    default:
      std::cerr << "Data corruption detected: Invalid TPacketType flag."
                << std::endl;
  }
...
void TPacket::print() {
  std::cerr << "Timesync: from_sent: " << from_sent << " to_recvd: " << to_recvd
            << " to_sent: " << to_sent << " from_recvd: " << from_recvd
